	...
	protected function getListQuery()
	{
		...
		$query->select(
			$this->getState(
				'list.select',
				'a.*,
				(SELECT MAX(`created`) FROM #__gilla_events_users WHERE events_id = a.id) AS last_comment,
				(SELECT COUNT(`created`) FROM #__gilla_events_users WHERE events_id = a.id) AS ncomments
				')
		);
		...
		if (!empty($search))
		{
			...
			$searches[]	= 'a.description LIKE '.$search;
			// Add search clauses to query.
			$query->where('('.implode(' OR ', $searches).')');
		}

		// Add the list ordering clause.
		$orderCol  = $this->state->get('list.ordering', 'a.date_time');
